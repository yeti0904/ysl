	...
	namespace Extensions {
		namespace Posix {
			std::vector <int> Execv(const std::vector <std::string>& args, Environment& env) {
				...
				auto argv = Util::StringVectorToCharpVector(
					Util::IntVectorToStringVector(env.variables[args[1]])
				);
			...
			std::vector <int> Execvpe(const std::vector <std::string>& args, Environment& env) {
				...
				auto envp = Util::StringVectorToCharpVector(
					Util::IntVectorToStringVector(env.variables[args[1]])
				);
			...
			std::vector <int> Close(const std::vector <std::string>& args, Environment& env) {
				...
				env.Assert(Util::IsInteger(args[0]), "Posix.Close: Invalid FD");
			...
			std::vector <int> Read(const std::vector <std::string>& args, Environment& env) {
				...
				env.Assert(Util::IsInteger(args[0]), "Posix.Read: Invalid FD");
				env.Assert(
					Util::IsInteger(args[2]), "Posix.Read: Count must be integer"
				);

				char*   buf = (char*) malloc(stoi(args[2]));
				ssize_t readRet = read(stoi(args[0]), &buf, stoi(args[2]));

				auto ret = Util::StringToIntVector(buf);
				free(buf);
